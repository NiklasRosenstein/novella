...
class PythonAction(Action):
  ...
  * A function argument in Python 3 cannot be called `print` even though it is legal syntax.
  ...
  def get_effective_search_path(self, context: Context) -> list[str]:
    search_path = list(self.search_path)
    if '*' in search_path:
      index = search_path.index('*')
      search_path[index:index+1] = sys.path
    return [os.path.join(context.project_directory, x) for x in search_path] + (sys.path if self.use_sys_path else [])

  def load(self, context: Context) -> t.Iterable[docspec.Module]:
